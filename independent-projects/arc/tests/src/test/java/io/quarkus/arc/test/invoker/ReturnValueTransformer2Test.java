...
public class ReturnValueTransformer2Test {
    @RegisterExtension
    public ArcTestContainer container = ArcTestContainer.builder()
            ...
            .beanRegistrars(new InvokerHelperRegistrar(MyService.class, (bean, invokers) -> {
                for (MethodInfo invokableMethod : bean.getInvokableMethods()) {
                    if (invokableMethod.name().equals("hello")) {
                        invokers.accept(invokableMethod.name(), bean.createInvoker(invokableMethod)
                                .setReturnValueTransformer(StringTransformer.class)
                                .build());
                    } else if (invokableMethod.name().equals("doSomething")) {
                        invokers.accept(invokableMethod.name(), bean.createInvoker(invokableMethod)
                                .setReturnValueTransformer(SetOfStringsTransformer.class)
                                .build());
                    }
                }
            }))
    ...
    static class StringTransformer implements Transformer<String, String> {
        @Override
    ...
    static class SetOfStringsTransformer implements Transformer<Set<String>, Set<String>> {
        @Override
        public Set<String> transform(Set<String> value) {
            return value.stream().map(it -> it.toUpperCase(Locale.ROOT)).collect(Collectors.toSet());
        }
    }
